...
export class OrdersValidationsController {
  ...
  @Post()
  create(
    @Body() createOrderValidationDto: CreateOrderValidationDto,
  ...
  findAll(
    ...
    const filters: { orderId?: number; validatedById?: number } = {};
    if (orderId) filters.orderId = parseInt(orderId, 10);
    if (validatedById) filters.validatedById = parseInt(validatedById, 10);
